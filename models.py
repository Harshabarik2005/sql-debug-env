...
class SQLAction(_ActionBase):
    ...
    # BUG FIX: extra='ignore' so unknown fields from openenv-core don't crash
...
class SQLObservation(_ObsBase):
    ...
    # BUG FIX: extra='ignore' so fields added by openenv-core base class
    # (done, reward, metadata) don't cause Pydantic validation errors
    model_config = ConfigDict(extra="ignore")
...
class SQLState(_StateBase):
    ...
    # BUG FIX: extra='ignore' for same reason as above
    model_config = ConfigDict(extra="ignore")
