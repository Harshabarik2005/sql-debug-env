...
MANDATORY environment variables:
    ...
    SQL_DEBUG_ENV_URL   Direct server URL (default: live HF Space URL)

NOTE: IMAGE_NAME is intentionally ignored. The competition validator sets
IMAGE_NAME but we always connect via HTTP URL to avoid Docker failures.
...
# ---------------------------------------------------------------------------
# Configuration
# FIX 1: IMAGE_NAME completely ignored — always connect via URL
# FIX 2: API_KEY never None — defaults to dummy-key so OpenAI never crashes
# FIX 3: ENV_URL defaults to live HF Space, NOT localhost
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Logging — stdout only, always flush=True
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Single episode
# FIX 4: log_start called first before anything can fail
# FIX 5: log_end always called via finally block
# FIX 6: obs initialised to None, score only read if obs is not None
# ---------------------------------------------------------------------------
async def run_episode(env: SQLDebugEnv, task_id: str, llm_client: OpenAI) -> dict:
    ...
    try:
        ...
        if obs is not None and not success:
            score = obs.current_best_score
...
# ---------------------------------------------------------------------------
# Main
# FIX 7: entire main() wrapped in try/except — never crashes with unhandled exception
# FIX 8: on fatal setup failure, still emit [START]/[END] for every task
# FIX 9: each task individually protected — one failure never stops others
# FIX 10: env.close() always called safely in finally
# FIX 11: IMAGE_NAME completely removed — always use ENV_URL
# ---------------------------------------------------------------------------
async def main() -> None:
    ...
    try:
        ...
        # Emit structured output for all remaining tasks
        for task_id in TASK_IDS_ORDERED:
